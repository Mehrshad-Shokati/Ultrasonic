...
Nsi = 50; % Reduced number of sampling points for 'si'
N_phi_max = 50; % Reduced number of angular samples
si_max = pi/3; % Maximum angle for the simulation
element_Number = 20; % Reduced number of elements in the annular array
...
% Visualize the transducer array layout to identify proper element positions
figure;
polarplot(element_alfa, ones(1, element_Number), 'o');
title('Transducer Array Layout');
ax = gca;
ax.ThetaTickLabel = arrayfun(@num2str, 1:element_Number, 'UniformOutput', false);
hold on;
...
vertical_indices = [7, 8, 9, 10, 11, 12, 13]; % Adjusted for better vertical line

% Define the indices for the horizontal part of the "T"
horizontal_indices = [9, 10, 11, 3, 17]; % Adjusted for a more visible horizontal line

% Set the excitation for the "T" pattern
element_ui(vertical_indices) = 2.0;  % Strong excitation for the vertical part
element_ui(horizontal_indices) = 2.0; % Strong excitation for the horizontal part

% Highlight the selected elements on the layout
polarplot(element_alfa(vertical_indices), ones(1, length(vertical_indices)), 'xr', 'MarkerSize', 10);
polarplot(element_alfa(horizontal_indices), ones(1, length(horizontal_indices)), 'xb', 'MarkerSize', 10);
legend('Array Elements', 'Vertical Part', 'Horizontal Part');

disp('Element excitation set.');

% Number of subdivisions for element integration
Nsr = 5; % Reduced to speed up execution
...
for i = 1:Nsi
    ...
    for j = 1:N_phi_max
        ...
        if mod(counter, 100) == 0 % Show progress every 100 calculations
            disp(['Calculation step: ', num2str(counter)]);
        end
...
disp(['Pressure field calculation complete. Total steps: ', num2str(counter)]);
